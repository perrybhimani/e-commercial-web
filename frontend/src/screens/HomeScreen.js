...
function HomeScreen(props) {
  ...
  var queryparmas = props.location.search.split("q=")[props.location.search.split("q=").length-1] || ' ';
      ...
      <ul className="filter">
        <li>
          ...
          <select name="sortOrder" value={sortOrder} onChange={sortHandler}>
            ...
            <option value="lowest" default>Lowest</option>
          ...
          {products.map((product) => (
            <li key={product._id}>
              <div className="product">
                <Link to={'/product/' + product._id} onClick={() => { datalayer(product) }}>
                  <img
                ...
                <div className="product-name">
                  <Link to={'/product/' + product._id} onClick={() => { datalayer(product) }}>{product.name}</Link>
                </div>
